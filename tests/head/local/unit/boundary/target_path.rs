...
fn target_path_normalizes_dot_segments_and_keeps_repeated_slashes() {
    let normalized = HeadBuilder::new(client(), base_url())
        .target_path("a/./b/../c")
        .expect("合法相对路径应被接受")
        .build()
        .expect("请求应构建成功");
    let repeated = HeadBuilder::new(client(), base_url())
        .target_path("a//b///c")
        .expect("合法相对路径应被接受")
        .build()
        .expect("请求应构建成功");
...
fn target_path_rejects_malformed_input() {
    let error = HeadBuilder::new(client(), base_url())
        .target_path("http://[::1")
        .expect_err("非法 URL 应被拒绝");

    assert!(matches!(error, HeadError::Url(_)));
}
...
fn target_url_replaces_auth_root() {
    ...
    assert_eq!(request.url().as_str(), "https://other.example.com/file.txt");
}
