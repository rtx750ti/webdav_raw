...
use webdav_core::{HeaderMap, HeaderValue, MoveBuilder, MoveDepth, Overwrite};

use crate::support::fixtures::{base_url, client};

/// 未设置任何地址时源是认证根地址；方法为 MOVE。
#[test]
fn default_source_is_auth_root_and_method_is_move() {
...
/// `MoveDepth::Zero` 与 `One` 如实写进 `Depth` 头。
#[test]
fn explicit_depths_reach_the_header() {
    let zero = MoveBuilder::new(client(), base_url())
        .target_path("t.txt")
        .expect("合法相对路径应被接受")
        .depth(MoveDepth::Zero)
        .build()
        .expect("请求应构建成功");
    let one = MoveBuilder::new(client(), base_url())
        .target_path("t.txt")
        .expect("合法相对路径应被接受")
        .depth(MoveDepth::One)
        .build()
...
/// `Overwrite::False` 时写出 `Overwrite: F`；设回 `True` 后该头消失。
#[test]
fn overwrite_false_writes_header_and_true_removes_it() {
    let refused = MoveBuilder::new(client(), base_url())
        .target_path("t.txt")
        .expect("合法相对路径应被接受")
        .overwrite(Overwrite::False)
        .build()
        .expect("请求应构建成功");
    let allowed = MoveBuilder::new(client(), base_url())
        .target_path("t.txt")
        .expect("合法相对路径应被接受")
        .overwrite(Overwrite::False)
    ...
    assert_eq!(refused.headers().get("overwrite").unwrap(), "F");
    assert!(allowed.headers().get("overwrite").is_none());
}
